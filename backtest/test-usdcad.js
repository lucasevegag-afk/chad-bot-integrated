/**
 * backtest/test-usdcad.js
 * Test estrategias del registry sobre USDCAD.
 */
const { spawnSync } = require('child_process');
const path = require('path');
const { STRATEGIES } = require('../server/bot/strategies/registry');

// Las base 4 + S8 (índices) para ver si transfiere a FX
...
function runOne(env) {
  const child = spawnSync('node', [path.join(__dirname, 'run.js'), 'USDCAD'], { env, encoding: 'utf8' });
...
console.log('🧪 TESTING 5 STRATEGIES ON USDCAD\n');
...
for (const strat of TO_TEST) {
  ...
  const decay = periodResults.TRAIN.avgR !== 0
    ...
    : 0;
  results.push({ id: strat.id, full: periodResults.FULL, train: periodResults.TRAIN, test: periodResults.TEST, decay });
}

console.log('\n\n' + '═'.repeat(120));
console.log('📊 USDCAD — transferencia de estrategias');
